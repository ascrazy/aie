...
async function main() {
  ...
  const res = await pg.query<DBMessage>('SELECT id, text FROM messages ORDER BY embeddings <#> $1 LIMIT $2', [
    JSON.stringify(document),
    contextSize,
  ])

  const model = new OpenAI({ openAIApiKey: config.openAIApiKey, temperature: 0.9 });
  const ans = await model.call(`Context:\n${res.rows.map(row => row.text).join('\n-----\n')}\nQuestion:\n${question}`)

  console.table(res.rows.map(r => [r.id, r.text.replace('\n', ' ').slice(0, 100)]))
  console.log(ans)
